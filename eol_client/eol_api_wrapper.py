...
class Search(object):
    ...
    def __init__(self, q, page=1, exact=False, filter_by_taxon_concept_id='', filter_by_hierarchy_entry_id='',
        ...
        attributes = [q, page, API._bool_converter(
            exact), filter_by_taxon_concept_id, filter_by_hierarchy_entry_id, filter_by_string, cache_ttl, key]
...
class Collection(object):
    ...
    def __init__(self, id, page=1, per_page=50, filter='none', sort_by='recently_added', sort_field='', cache_ttl='', key=''):
        ...
        attributes = [id, page, per_page, filter,
                      sort_by, sort_field, cache_ttl, key]
        self.key = key
...
class DataObject(object):
    ...
    def __init__(self, id, cache_ttl='', key=''):
        ...
        self.key = key
        url = "http://eol.org/api/data_objects/1.0/{}.json?cache_ttl={}&key={}".format(
            *attributes)
...
class Hierachy_entries(object):
    ...
    def __init__(self, id, common_names=False, synonyms=False, cache_ttl='', key=''):
        attributes = [id, API._bool_converter(
            common_names), API._bool_converter(synonyms), cache_ttl, key]
        url = "http://eol.org/api/hierarchy_entries/1.0/{}.json?common_names={}&synonyms={}&cache_ttl={}&key={}".format(
            *attributes)
